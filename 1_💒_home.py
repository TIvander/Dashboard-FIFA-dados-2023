...
if "data" not in st.session_state:
    ...
    df_data["Contract Valid Until"] = df_data["Contract Valid Until"].apply( lambda x : str(x).split(".")[0])
    df_data = df_data[df_data["Contract Valid Until"] >= '2023']
    df_data.sort_values(by="Overall", ascending=False)
    st.session_state["data"] = df_data

st.markdown("# FIFA23 OFFICIAL DATASET! ⚽️")
st.sidebar.markdown("Desenvolvido através das aulas em: [Asimov Academy](https://asimov.academy)")
